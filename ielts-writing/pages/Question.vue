<template>
  <div class="">
    ...
    <div class="my-10 mx-20 space-y-1">
      <div v-for="(category, index) in questions" :key="index" class="">
        ...
        <ul class="grid grid-cols-1 lg:grid-cols-2 gap-x-16">
          <li v-for="(q, qIndex) in category.questions" :key="qIndex" class="mb-2 border-2 drop-shadow-lg p-5 rounded-lg">
            {{ q }}
          </li>
        </ul>
      </div>
...
onMounted(async () => {
  try {
    ...
    console.log("Loaded Data:", questions.value); // Debugging Output
  } catch (error) {
