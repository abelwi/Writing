<template>
    <div class="h-screen bg-orange-100 pt-5">
      ...
      <div class="p-5 flex space-x-10">
        ...
        <div class="w-1/3">
          <div v-if="state.apiResult.scoringResult && state.apiResult.scoringResult.overallBand" class="grid grid-cols-5 gap-x-1 text-center mb-10 bg-base-100 py-4 px-5 rounded-xl">
          ...
          <div>
            ...
            <div v-if="activeTab === 1" id="panel-1" ref="panel1" role="tabpanel" aria-labelledby="tab-1" class="px-5 pb-10 pt-8 border-2 border-base-300 rounded-b-lg mt-[-2px] space-y-8 overflow-y-auto bg-base-100" style="max-height: calc(75vh - 120px);">
              <div v-if="state.apiResult.commentResult">
                <p>{{ state.apiResult.commentResult.overallComment }}</p>
              </div>
            ...
            <div v-if="state.apiResult.commentResult">
              <div v-if="activeTab === 2" id="panel-2" ref="panel2" role="tabpanel" aria-labelledby="tab-2" class="px-5 pt-6 pb-10 border-2 border-base-300 rounded-b-lg mt-[-2px] space-y-5 overflow-y-auto bg-base-100" style="max-height: calc(75vh - 120px);">
                <p class="font-semibold border border-orange-500 w-1/2 p-2 text-center text-orange-500 rounded-lg">Task Response</p>
                <p>{{ state.apiResult.commentResult.taskAchievement?.comment }}</p>
  
                <p class="font-semibold border border-orange-500 w-1/2 p-2 text-center text-orange-500 rounded-lg">Coherence & Cohesion</p>
                <p>{{ state.apiResult.commentResult.coherenceCohesion?.comment }}</p>
  
                <p class="font-semibold border border-orange-500 w-1/2 p-2 text-center text-orange-500 rounded-lg">Lexical Resource</p>
                <p>{{ state.apiResult.commentResult.lexicalResource?.comment }}</p>
                ...
                <p>{{ state.apiResult.commentResult.grammaticalRangeAccuracy?.comment }}</p>
              </div>
            </div>
          </div>
        </div>
